...
2.  ただし、ユーザーの行為が以下のような場合には、当団体は事前通知または警告なしに直ちに利用制限または登録抹यो行うことがあります。
...
1.  当団体は、本サービスの利用によって取得する個人情報を、「kogamaiGTAプライバシーポリシー」に従い、適切に取り扱います。プライバシーポリシーの詳細については、以下のリンクからご覧いただけます
...
1.  ユーザーは任意で、当団体が指定する方法（Tebexを通じたPayPalやクレジットカード）により、月額のサブスクリプションまたは一括支援を行うことができます。
2.  支援により得られるのは、デジタルコンテンツの使用ライセンスであり、当該コンテンツの所有権を取得するものではありません。
3.  提供されるデジタルコンテンツは、ゲーム内で特定の条件（例：ゲーム内通貨による購入）を満たすことで利用可能となる場合があります。
4.  支援によって取得したライセンスは、契約期間中に限り有効であり、支援が終了した場合はその利用権も終了します。

## 第11条（返金ポリシー）

-   本サービスでは、支援および購入に対する返金は一切受け付けておりません。ユーザーは、支援・購入によりデジタルコンテンツの使用ライセンスを取得するものであり、コンテンツ自体の所有権を取得するものではありません。
-   本取引は最終的なものであり、いかなる理由によっても返金は行われません。ユーザーが本サービスのルールに違反してBAN（利用停止）措置を受けた場合も、返金は行われません。
-   当団体は、独自の裁量によりBAN措置を講じる権利を有し、また本サービスのルールは予告なく変更される場合があります。
-   本サービスへのアクセスは永続的に保証されるものではなく、サービスが終了した場合、すべてのデジタルコンテンツおよびライセンスは失効し、現実世界の価値を持たないため、返金や補償の対象にはなりません。
-   デジタルコンテンツはすべて仮想アイテムであり、現実世界の通貨と交換することはできません。
